/*
	...
	gcc -c hostd-ra.c -o hostd-ra
	./hostd-ra combined.txt
...
//  3. Fill dispatcher queue from dispatch list file;
    ...
    while (!feof(inputliststream)) {
        ...
        if (fscanf(inputliststream,"%d, %d, %d, %d, %d, %d, %d, %d",&(process->arrivaltime), &(process->priority),&(process->remainingcputime), &(process->mbytes),&(process->req.printers), &(process->req.scanners),&(process->req.modems), &(process->req.cds)) != 8) {
            free(process);
            continue;
        }
